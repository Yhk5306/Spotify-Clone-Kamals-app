<template>
   <div class="grid-container" :style="'--column-count:'+column+';'">
     <div class="card-container" v-for="artistAlbums in removeDuplicates(albums.items.slice(0,column))">
       <div class="card-height">
         <div class="card-img">
           <div class="card-img-arranger">
            <img :src="artistAlbums.images[0].url" :alt="artistAlbums.name">
           </div>
           <div class="card-btn">
             <div class="shrink">
         ...
         <div class="card-title">
           ...
           <div class="release-date">
            <time datetime="2003">{{ artistAlbums.release_date.slice(0,4) }}</time>
             <span class="type">
                Album
             </span>
           </div>
         </div>
         <div class="card-click-handler" @click="navigateTo({name:'album',params:{albumId:artistAlbums.id}}) "></div>
       </div>
     </div>
   </div>
</template>
...
<script>
export default {
  props:['albums','column'], //client/src/views/ArtistDetail.vue
  methods:{
    changeSong(uri,trackNumber) {
      this.$parent.changeSong(uri, trackNumber)
    },
    //There is a problem with this function data duplicates
